...
export default function FleetPage() {
  ...
  async function runAAIAFleetAnalysis() {
    ...
    try {
      ...
      const text = res.analysis || 'AAIA analyzed the fleet but found no pressing issues.';
      setAnalysis(text);
      
      const fleetName = fleets.find(f => f.id === activeFleetId)?.name || 'Unknown Fleet';
      const savedReport = await APIService.saveReport(`Deep Fleet Analysis - ${fleetName}`, 'Fleet', text);
  ...
  const downloadFleetReport = async () => {
    ...
    try {
      ...
      if (pdfTemplateRef.current) {
        ...
        pdf.save(`AAIA_${generatedReport.name.replace(/\s+/g, '_')}.pdf`);
      }
    } catch (err) {
      console.error(err);
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: 30, borderBottom: '2px solid #eee', paddingBottom: 20 }}>
              ...
              <div style={{ marginLeft: 'auto', textAlign: 'right' }}>
                <div style={{ fontSize: 12, color: '#666', textTransform: 'uppercase', letterSpacing: 1 }}>{generatedReport.type} Report</div>
                <div style={{ fontSize: 14, fontWeight: 600 }}>{new Date(generatedReport.created_at).toLocaleDateString()}</div>
              </div>
            </div>
            <h2 style={{ fontSize: 20, marginBottom: 20, color: '#222' }}>{generatedReport.name}</h2>
            ...
            <div className="card-body">
              {addMode === 'vin' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--sgray)' }}>AAIA will automatically decode the VIN to fetch make, model, year, and specifications.</div>
                </div>
              )}
              ...
              {addMode === 'bulk' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--sgray)' }}>File must contain a column named "VIN". AAIA will process and decode vehicles automatically in the background.</div>
                </div>
              )}
            </div>
