  ...
  async function handleStartDiagnosis() {
    ...
    try {
      ...
      if (image) {
        ...
        textContentForReport = res.assessment || 'No assessment generated.';
        reportName = `Vision Assessment - ${vin || 'Unknown'}`;
      } else {
        ...
        textContentForReport = `# DTC Definition\n${res.dtcDefinition || 'N/A'}\n\n# Diagnostic Summary\n${res.detailedSummary || 'N/A'}\n\n# Reasoning Nodes\n` + (res.nodes || []).map(n => `- **${n.title}**: ${n.description}`).join('\n');
        reportName = `Cognitive Diagnosis - ${vin || 'Unknown'}`;
      }
      
      const savedReport = await APIService.saveReport(reportName, 'Diagnostics', textContentForReport);
